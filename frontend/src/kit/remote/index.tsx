...
export type Remote<E, T> = Initial | Pending | Success<T> | Failure<E>
...
export const map =
	<A, B>(ab: (a: A) => B) =>
	<E extends any>(fa: Remote<E, A>): Remote<E, B> =>
		isSuccess(fa) ? success(ab(fa.value)) : fa

export const mapFailure =
	<E, EB>(eeb: (e: E) => EB) =>
	<A extends any>(fa: Remote<E, A>): Remote<EB, A> =>
		isFailure(fa) ? failure(eeb(fa.details)) : fa

// -------------------------------------------------------------------------------------
// Interop
// -------------------------------------------------------------------------------------
export const fromQueryState = <E, A>(
	state: UseQueryResult<AxiosResponse<A>, E>,
): Remote<E, A> => {
	switch (state.status) {
		case "idle":
			return initial
