...
const satisfy: {
	<A, B extends A>(refinement: Refinement<A, B>, message: string): Validation<
		B,
		A
	>
	...
	decode: (value: any) =>
		predicate(value) ? D2.success(value) : D2.failure(value, message),
})

export const eq = <A>(expected: A, Eq: EQ.Eq<A>): Validation<A, unknown> =>
	pipe(
		D2.fromRefinement(
			(actual): actual is A => Eq.equals(expected, actual as A),
			`Must be ${expected}`,
		),
		required,
	)
...
export const gte = (n: number) =>
	satisfy((value: number) => value >= n, `Must be more than or equal to "${n}"`)

export const min = gte

export const lte = (n: number) =>
	satisfy((value: number) => value <= n, `Must be less than or equal to "${n}"`)

export const max = lte

export const between = (min: number, max: number) => andThen(gte(min), lte(max))

type ValuesOf<T extends { [key: string]: any }> = T extends {
	[key: string]: infer V
}
	? V
	: never

export const enumm = <T extends { [name: string]: any }>(
	_enm: T,
): Validation<ValuesOf<T>, string> => {
...
export const andThen = <A, B, C>(
	ab: Validation<B, A>,
	bc: Validation<C, B>,
): Validation<C, A> => pipe(ab, D2.compose(bc))

export const contramap = <A, B>(
	map: (a: A) => B,
	validation: Validation<B, any>,
): Validation<A, A> => ({
...
export const map =
	<A, B, C>(map: (b: B) => C) =>
	(validation: Validation<B, A>): Validation<C, A> => ({
		decode: flow(validation.decode, EI.map(map)),
	})

export const optional = <A, B>(
	decoder: Validation<B, A>,
): Validation<B | undefined, A> => ({
	decode: (value: any) =>
...
export const required = <A, B>(
	validation: Validation<B, A>,
): Validation<NonNullable<B>, A> => ({
	decode: (value: A) =>
		value === null || value === undefined
...
type Slot<A, B extends A = A> = [
	Refinement<A, B> | Predicate<A>,
	Validation<B, unknown>,
]

export const cond = <A extends any>(
	slots: Slot<A>[],
): Validation<A, unknown> => ({
	decode: value => {
		const decoder = slots.find(s => s[0](value as any))
		return decoder ? decoder[1].decode(value) : D.success(value as A)
	},
})
...
export const date = pipe(
	satisfy((thing): thing is Date => thing instanceof Date, "Must be a date"),
	required,
)
...
export const nil = satisfy(
	(thing): thing is undefined => thing === undefined || thing === null,
	"Must be empty",
)

export const nonEmptyString = andThen(
	D2.string,
	satisfy(str => str.length > 0, "Cannot be blank"),
)

export const stringNumber = pipe(
	nonEmptyString,
	D2.parse((a: string) => {
		const parsed = parseInt(a)
		return isNaN(parsed) ? D2.failure(a, "Nan") : D2.success(parsed)
	}),
	required,
)
...
// Interpreters
// -------------------------------------------------------------------------------------
export const run = <B, A>(
	validation: Validation<B, A>,
	value: A,
): EI.Either<Failure[], B> => pipe(validation.decode(value), EI.mapLeft(report))

// -------------------------------------------------------------------------------------
// Interop
// -------------------------------------------------------------------------------------
export const asRefinement =
	<B extends A, A>(validation: Validation<B, A>): Refinement<A, B> =>
	(value: A): value is B =>
		EI.isRight(validation.decode(value))
