...
export const RadioGroup = <T extends Connectable>(
...
type RadioProps<T extends Connectable> = PropsWithChildren<
	FieldConnection<T>
...
export const CheckableRadio2 = <T extends Connectable>({
	onChange,
	...props
}: RadioProps<T>) => {
	const [field, meta, helpers] = useField({ ...props, type: "radio" })
...
export const Select2 = <T extends Connectable>(
	props: PropsWithChildren<
		FieldConnection<T> & {
			data: REMOTE.Remote<any, Code<T>[]>
			label: string
...
export const Input2 = <T extends number | string>({
	className,
	...props
}: FieldConnection<T> &
