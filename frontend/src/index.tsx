...
import { DateInput, NumberInput, YesNo } from "./kit-2/forms"
import * as V from "./kit-2/validation"
import { pick } from "./kit-2/helpers"
import * as UF2 from "./kit-2/forms/use-form-2"
import { useDatesAntecedentsSinistralites } from "./queries/drivers"
...
const struct = {
	...
	sinistres:                          UF2.list( UF2.record( {
		...
		codeResponsabilite: UF2.field( V.string ),
	} ) ),
}
...
const PasseAssure: PasseAssureStep = ( props ) => {
	...
	const [ autoDateAnterioriteBonus050 ] = useDatesAntecedentsSinistralites(
	...
	const [ isAutoDateAnterioriteCoefficientValid, setIsAutoDateCoefficientValid ] = useState<boolean | undefined>( undefined )
	
	const handleChangedAccompaniedDriving = ( answer: boolean ) => {
		fields.conduiteAccompagnee.set( true )
		if ( !answer ) form.setValues( {
	...
	const handleChangedAutoDateAnterioriteCoefficientValid = ( answer: boolean ) => {
		setIsAutoDateCoefficientValid( answer )
		
		REMOTE.isSuccess( autoDateAnterioriteBonus050 ) &&
		fields.dateAnterioriteBonus050.set(
			answer ?
	...
	const showCustomDateCoefficinentQuestion = showIsAutoDateCoefficinentValidQuestion && isAutoDateAnterioriteCoefficientValid === false
		...
		<form {...form.props}>
			...
			{showIsAutoDateCoefficinentValidQuestion && REMOTE.isSuccess( autoDateAnterioriteBonus050 ) && (
				<YesNo
					...
					name="isAutoDateCoefficinentValid"
					value={isAutoDateAnterioriteCoefficientValid}
