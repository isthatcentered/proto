...
const schema = Y.struct({
	...
	dateEffetContratDesiree: pipe(
		Y.dateString(),
		Y.minDateString(DS.today(), "Ne peut démarrer avant aujourd'hui"),
	),
})
...
const UsageVehicule = makeStep<UsageVehiculeStep, typeof schema>(
	props => {
		...
		const codesTypesUtilisationVehicule = Q.useCodesUtilisationVehicule(
			props.numeroRepertoire,
		)
