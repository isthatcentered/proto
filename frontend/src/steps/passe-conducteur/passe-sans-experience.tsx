...
const PasseSansExperience = makeStep<PasseConducteurStep, typeof caSchema>(
	props => {
		...
		const showConduiteAccompaniedDrivingWithMaifQuestion =
			props.values.conduiteAccompagnee === "true"
		const showConduiteAccompaniedDrivingBefore2007Question =
			props.values.conduiteAccompagnee === "true" &&
			props.values.conduiteAccompagneeMaif === "true"
		...
		const handleCAMAIFChanged = BS.fold({
			onTrue: () => {
				...
				props.setTouched(
					{ ...props.touched, conduiteAccompagneeMaif: true },
					false,
				)
			},
			onFalse: () => {
